...
function renderPhotos(){$('photos').innerHTML='';for(let i=0;i<10;i++){const d=document.createElement('div');d.className='photo';if(evidenceImages[i]){const im=new Image();im.src=evidenceImages[i];d.appendChild(im)}else d.textContent=`الشاهد ${i+1}`;$('photos').appendChild(d)}}renderPhotos();
$('images').addEventListener('change',async()=>{evidenceImages=[];show('جارٍ تجهيز صور الشواهد...');for(const f of [...$('images').files].slice(0,10)){try{evidenceImages.push(await compressImage(f))}catch(e){}}renderPhotos();show(`تم تجهيز ${evidenceImages.length} من 10 شواهد.`)});
